...
(defn range-overlap [[a-start a-end :as a] [b-start b-end :as b]]
  (when (> a-start b-start)
    (throw (Exception. (str "Whyyy" a b))))
  (when (not (neg? (- a-end b-start)))
...
(comment
  (range-overlap [1 7] [2 5])
  (range-overlap [6 7] [8 12])
  (range-overlap [4 7] [5 7]))
...
(defn merge-ranges [{:keys [set] :as acc} [[this-start this-end] this-styles :as attribute]]
  (let [sorted-set (sort range-sort set) 
    ...
    (cond-> acc
        ...
        (seq post)    (update :post conj [post (cond-> {}
        ...
        (nil? (seq overlap)) (update :set conj attribute))))

(loop [{:keys [set post] :as acc} {:set (list) :post attributes}]
  (if (seq post)
    (as-> acc $
      (into (:set $) (:post $))
      (sort range-sort $)
